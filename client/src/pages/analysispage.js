...
const AnalysisPage = ({ sessionUser, onLogout, setSessionUser }) => {
  ...
  const handleDeleteSelected = async () => {
    ...
    /* -----------------------------------------------------------------       
       try {
         // 1. 백엔드 삭제 요청 (예: POST /image/delete)
         // 전달 데이터: { "ids": [1, 2, 3] }
         const response = await axios.post('http://localhost:8000/image/delete/image_id', { -> get
           ids: selectedIds 
         });

         if (response.data.status === "success") {
           alert("서버에서 성공적으로 삭제되었습니다.");
           fetchHistory(); // 서버 데이터 다시 불러오기
         }
       } catch (error) {
         alert("서버 통신 중 오류가 발생했습니다.");
         return; // 서버 에러 시 아래 프론트 필터링 로직을 수행하지 않음
       }
       ----------------------------------------------------------------- */
    ...
    setSelectedIds([]);
    setIsEditMode(false);
    alert("화면에서 제거되었습니다. (백엔드 연동 전까지는 새로고침 시 복구됩니다)");
  };
  ...
  return (
    <div style={{ display: 'flex', backgroundColor: '#000', height: '100vh', width: '100vw', color: 'white', fontFamily: 'sans-serif', overflow: 'hidden' }}>
      <aside style={sideBarStyle}>
        <button onClick={() => { setFile(null); setPreviewUrl(null); setResult(null); setShowOptions(false); }} style={{ backgroundColor: '#1A2C50', color: 'white', padding: '14px', borderRadius: '10px', border: 'none', marginBottom: '35px', cursor: 'pointer', fontWeight: 'bold' }}>+ 새 프로젝트 시작</button>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          ...
          {sessionUser && history.length > 0 && (
            <button 
              ...
              style={{ background: 'none', border: 'none', color: '#39FF14', cursor: 'pointer', fontSize: '12px' }}
            >
              {isEditMode ? '취소' : '편집'}
            </button>
          )}
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {sessionUser ? (
            history.map((item, index) => {
              ...
              const displaySettings = `${vType} | ${dType} | ${mType}`;

              const transportState = {
                ...item,
                prob: prob,
                face_conf: item.face_conf ?? item.face_confidence ?? item.conf ?? item.analysis?.face_conf ?? 0,
                face_ratio: item.face_ratio ?? item.ratio ?? item.analysis?.face_ratio ?? 0,
                face_brightness: item.face_brightness ?? item.brightness ?? item.analysis?.face_brightness ?? 0,
                label: item.label || (probNum !== -1 ? (probNum > 0.5 ? 'FAKE' : 'REAL') : 'UNKNOWN')
              };
              ...
              return (
                <div key={item.id || index} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '15px' }}>
                  ...
                  <div 
                    onClick={() => !isEditMode && navigate('/analysis-detail', { state: transportState })} 
                    style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '15px', padding: '12px', backgroundColor: '#111', borderRadius: '12px', border: selectedIds.includes(item.id) ? '1px solid #39FF14' : '1px solid #222', cursor: isEditMode ? 'default' : 'pointer' }}
                  >
                    ...
                    <div>
                      <div style={{ fontSize: '12px', color: '#aaa', fontWeight: 'bold', marginBottom: '2px' }}>{displaySettings}</div>
      ...
      <main style={centerZoneStyle}>
        ...
        <div style={{ display: 'flex', flex: 1, gap: '20px' }}>
          ...
          >
             {previewUrl ? (
               <img src={previewUrl} alt="preview" style={{ maxWidth: '95%', maxHeight: '95%', objectFit: 'contain' }} />
             ) : (
               <div style={{textAlign:'center', display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
                 <div style={plusBtnStyle}>+</div>
                 <p style={{fontSize:'20px', fontWeight:'bold', marginBottom: '8px'}}>Drag & Drop</p>
                 <p style={{color:'#555', fontSize:'14px'}}>또는 클릭하여 파일 선택</p>
                 {showOptions && (
                   <div style={{ marginTop: '25px', display: 'flex', gap: '12px' }}>
                     <button onClick={(e) => { e.stopPropagation(); document.getElementById('fileInput').click(); }} style={{ padding: '10px 18px', backgroundColor: '#222', color: '#39FF14', border: '1px solid #39FF14', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}>내 PC 파일</button>
                     <button onClick={(e) => { e.stopPropagation(); alert("Cloud Drive 연동 준비 중입니다."); setShowOptions(false); }} style={{ padding: '10px 18px', backgroundColor: '#222', color: '#fff', border: '1px solid #444', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}>Cloud Drive</button>
                   </div>
                 )}
               </div>
             )}
          ...
          <div style={{...innerBoxStyle, cursor: 'default'}}>
             {isAnalyzing ? <div style={{textAlign:'center'}}><p>분석 중...</p></div> : result ? (
               <div style={{textAlign:'center'}}>
                 <p style={{fontSize:'32px', fontWeight:'bold', color: result.analysis.prob > 0.5 ? '#FF4B4B' : (result.analysis.prob === -1 ? '#888' : '#39FF14')}}>{result.analysis.prob === -1 ? 'UNKNOWN' : (result.analysis.prob > 0.5 ? 'FAKE' : 'REAL')}</p>
                 <button onClick={() => navigate('/analysis-detail', { state: { ...result.analysis, image_loc: previewUrl, label: result.analysis.prob === -1 ? 'UNKNOWN' : (result.analysis.prob > 0.5 ? 'FAKE' : 'REAL'), message: result.message, version_type: versionType, domain_type: domainType } })} style={{ color: '#39FF14', background: 'none', border: 'none', textDecoration: 'underline', marginTop: '10px', cursor:'pointer' }}>상세 결과 보기</button>
               </div>
