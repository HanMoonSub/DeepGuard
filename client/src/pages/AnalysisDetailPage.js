...
const AnalysisDetailPage = ({ sessionUser }) => {
  ...
  const prob = data.prob ?? data.analysis?.prob ?? -1;
  const face_conf = data.face_conf ?? data.analysis?.face_conf ?? data.conf ?? 0;
        ...
        <div style={{ flex: 0.8, display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
            ...
            <div style={{ gridColumn: 'span 2', padding: '24px', backgroundColor: '#0D0D0D', borderRadius: '20px', border: '1px solid #1A1A1A' }}>
              ...
              <p style={{ fontSize: '14px', color: '#888', marginTop: '20px' }}>{data.message || "Deep Guard AI 분석 리포트입니다."}</p>
            </div>
          </div>
        </div>
      </div>
