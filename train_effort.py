...
# Effort wraps a frozen CLIP backbone, so inputs must use CLIP's own
# normalization statistics instead of DeepGuard's default ImageNet ones.
effort_train_transforms = partial(get_train_transforms, mean=CLIP_MEAN, std=CLIP_STD)
effort_valid_transforms = partial(get_valid_transforms, mean=CLIP_MEAN, std=CLIP_STD)
...
class Trainer:
    ...
    def __init__(self, model, cfg):
        ...
        # ======= Optimizer =======
        self.optimizer = build_effort_optimizer(self.model, cfg)
    ...
    def fit(self, train_df, valid_df):
        ...
        valid_ds = CutMixDeepFakeDataset(
                        ...
                        transforms = effort_valid_transforms,
                        mixup_prob = 0.,
                        cutout_prob = 0.,
                        )
        ...
        for epoch in range(1, self.cfg.train.epochs + 1):
            ...
            train_ds = CutMixDeepFakeDataset(
                        meta_df = balanced_df,
                        img_size = self.cfg.model.img_size,
                        transforms = effort_train_transforms,
...
def main():
    parser = argparse.ArgumentParser(description="Effort Training")

    parser.add_argument("--root-dir", required=True)
    parser.add_argument("--model-ver", default="effort", choices=["effort"])
