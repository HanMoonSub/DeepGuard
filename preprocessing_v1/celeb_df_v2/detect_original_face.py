...
def process_videos(
    ...
    pbar = tqdm(video_paths, total=len(video_paths), desc="Detecting Faces")
    ...
    for i, video_path in enumerate(pbar):
        ...
        frames_dict = frame_extractor.extract_frames_for_detect(video_path, num_frames)
...
def main():
    ...
    face_detector = FaceDetector(args.root_dir, args.conf_thres, args.min_face_ratio)
    frame_extractor = FrameExtractor(args.jitter)
    
    video_paths = get_original_video_paths(args.root_dir)
    
    process_videos(video_paths, args.root_dir, args.num_frames,
