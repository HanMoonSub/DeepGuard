...
from deepguard.data import (
    split_data,
    class_imbalance_handle
)
from deepguard.data import (
    CutMixDeepFakeDataset,
...
from deepguard.optimizer import (
    build_iid_optimizer
)
...
# IID's Xception/IResNet branches share the same input, normalized with this
# convention instead of DeepGuard's default ImageNet one.
iid_train_transforms = partial(get_train_transforms, mean=IID_MEAN, std=IID_STD)
iid_valid_transforms = partial(get_valid_transforms, mean=IID_MEAN, std=IID_STD)
...
class Trainer:
    ...
    def __init__(self, model, cfg):
        ...
        # ======= Optimizer =======
        self.optimizer = build_iid_optimizer(self.model, cfg)
    ...
    def _eic_loss(self, implicit_emb, explicit_emb, labels):
        """
        Explicit-Implicit identity Consistency loss.

        Pushes the cosine similarity between the trainable implicit embedding
        and the frozen explicit (ArcFace) embedding UP for real faces (label=0)
        and DOWN for fake/face-swapped faces (label=1) - real identity should
        be self-consistent, a face-swap should leave the two out of sync.
        """
        implicit_n = F.normalize(implicit_emb, dim=1)
        explicit_n = F.normalize(explicit_emb, dim=1)
        sim = (implicit_n * explicit_n).sum(dim=1)  # (B,) cosine similarity per sample

        real_mask = labels == 0
        fake_mask = labels == 1

        real_sim = sim[real_mask].mean() if real_mask.any() else sim.new_tensor(0.0)
        fake_sim = sim[fake_mask].mean() if fake_mask.any() else sim.new_tensor(0.0)

        return fake_sim - real_sim
    ...
    def fit(self, train_df, valid_df):
        ...
        valid_ds = CutMixDeepFakeDataset(
                        ...
                        transforms = iid_valid_transforms,
                        mixup_prob = 0.,
                        cutout_prob = 0.,
                        )
        ...
        for epoch in range(1, self.cfg.train.epochs + 1):
            ...
            train_ds = CutMixDeepFakeDataset(
                        meta_df = balanced_df,
                        img_size = self.cfg.model.img_size,
                        transforms = iid_train_transforms,
    ...
    def train_one_epoch(self, loader):
        ...
        for step, (imgs, labels) in pbar:
            ...
            with torch.amp.autocast(device_type="cuda", enabled=self.cfg.train.use_amp):
                out = self.model(imgs)
                logit = out['logit'].view(-1) # (B,)

                loss_ce = self.criterion(logit, labels)
                loss_eic = self._eic_loss(out['implicit_emb'], out['explicit_emb'], labels)
                loss = loss_ce + self.cfg.train.loss_eic_weight * loss_eic
            ...
            preds_sig = torch.sigmoid(logit).detach()
...
def main():
    parser = argparse.ArgumentParser(description="IID Training")

    parser.add_argument("--root-dir", required=True)
    parser.add_argument("--model-ver", default="iid", choices=["iid"])
    ...
    parser.add_argument("--explicit-extractor-path", default=None, type=str,
                         help="Local path to the pretrained ArcFace-style IResNet-50 "
                              "checkpoint. Overrides model.explicit_extractor_path in "
                              "the yaml config when given.")
    ...
    config_path = ROOT_DIR / "deepguard" / "config" / args.model_ver / f"{args.dataset}.yaml"

    cfg = OmegaConf.load(config_path)
    cfg.device = "cuda:0" if torch.cuda.is_available() else 'cpu'
    cfg.label_col = args.label_col
    if args.explicit_extractor_path is not None:
        cfg.model.explicit_extractor_path = args.explicit_extractor_path
